...
module Fluent
  module Plugin
    class Output < Base
      ...
      def write_count
        @write_count_metrics.get(self.plugin_id)
      end

      def rollback_count
        @rollback_count_metrics.get(self.plugin_id)
      end
      ...
      def rollback_write(chunk_id, update_retry: true)
        ...
        if @buffer.takeback_chunk(chunk_id)
          @rollback_count_metrics.inc(self.plugin_id)
          if update_retry
      ...
      def try_flush
        ...
        begin
          ...
          if output.delayed_commit
            ...
            @write_count_metrics.inc(self.plugin_id)
            @dequeued_chunks_mutex.synchronize do
      ...
      def check_slow_flush(start)
        ...
        @flush_time_count_metrics.add(self.plugin_id, elapsed_millsec)
        if elapsed_time > @slow_flush_log_threshold
          @slow_flush_count_metrics.inc(self.plugin_id)
      ...
      def statistics
        stats = {
          ...
          'retry_count' => @num_errors_metrics.get(self.plugin_id),
          'emit_count' => @emit_count_metrics.get(self.plugin_id),
          'write_count' => @write_count_metrics.get(self.plugin_id),
          'rollback_count' => @rollback_count_metrics.get(self.plugin_id),
          'slow_flush_count' => @slow_flush_count_metrics.get(self.plugin_id),
          'flush_time_count' => @flush_time_count_metrics.get(self.plugin_id),
        }
