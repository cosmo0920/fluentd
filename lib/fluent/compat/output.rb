  ...
  module Compat
    ...
    class BufferedOutput < Fluent::Plugin::Output
      ...
      def handle_stream_simple(tag, es, enqueue: false)
        if @overrides_emit
          current_emit_count = @emit_count_metrics.get(self.plugin_id)
          size = es.size
          key = data = nil
          begin
            emit(tag, es, NULL_OUTPUT_CHAIN)
            ...
            @emit_count_metrics.set(self.plugin_id, current_emit_count)
          ...
          @counter_mutex.synchronize do
            ...
            @emit_records_metrics.add(self.plugin_id, size)
          end
          return [meta]
        end
